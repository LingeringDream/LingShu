...
| Apple Calendar | 自然语言排程、L1 权限确认流、事件 CRUD、EventKit 写入系统日历、external_event_id 回写 | ✅ 后端 + 桥接完成 |
| 权限分级 | L0–L4 逐级系统权限 + 审计日志 | ✅ 后端完成 |
...
**核心后端机制已完成，进入桌面/原生集成收尾阶段。** SoulLedger 记忆系统、人格、思考队列、权限分级、向量检索、信号埋点、工具调用、日历 EventKit 写入等均已实现，后端经 **223 项测试通过（0 失败 / 15 ignored）**。

- ✅ Rust + Axum 后端（API、WebSocket、数据库迁移 001–0021）
...
- ✅ Apple Calendar：自然语言解析 + CRUD + L1 确认 + EventKit 写入 + external_event_id 回写
...
SoulLedger 关键模块（`crates/lingshu-server/src/llm/`）：`memory.rs`（抽取/去重）、`forgetting.rs`（衰减 + 来源护栏）、`personality.rs`（人格演化）、`consolidation.rs`（离线语义合并）、`thoughts.rs`（主动建议）、`dedup.rs`、`prompts.rs`、`semantic.rs`、`client.rs`（Ollama/OpenAI + 嵌入 + 工具调用）。
...
OpenAPI 3.0 由 [utoipa](https://github.com/juhaku/utoipa) 生成，覆盖全部 **34 个已注册路由**（系统、认证、用户、LLM 设置、对话、会话、记忆、人格、思绪、日历、权限、集成、信号、审计、项目管理等）。后端运行时访问 `/swagger-ui`。

## 已知局限

- WebSocket 仍为回声桩，主动建议的实时推送待实现。
- 权限分级运行时设置目前存内存，进程重启回默认（LLM 设置已于 2026-06-09 迁移至 PostgreSQL 持久化）。
