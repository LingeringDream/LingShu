...
|----------|-----------|--------|
| 桌面宠物 Desktop Pet | 透明浮层、可拖拽、贴边浮窗 Transparent overlay, draggable, floating pet window | ✅ Tauri 壳就位 Tauri shell in place |
| SoulLedger | 分层记忆、去重、遗忘衰减(含来源护栏)、人格演化、离线语义合并、向量检索 Layered memory, dedup, forgetting decay (with provenance guard), personality evolution, offline semantic consolidation, vector retrieval | ✅ 后端完成 Backend complete |
| Apple Calendar | 自然语言排程、L1 权限确认流、事件 CRUD Natural-language scheduling, L1 confirmation flow, event CRUD | ◐ 后端完成；EventKit 写入待 Phase B Backend complete; EventKit write pending |
| 权限分级 Permission Tiers | L0–L4 逐级系统权限 + 审计日志 Graduated system access with audit logging | ✅ 后端完成 Backend complete |
| 思考队列 Thought Queue | 状态机 + 防打扰抑制 + 每日维护扫描 + 可解释置信度 State machine, anti-nag suppression, daily maintenance sweep, explainable confidence | ✅ 后端完成 Backend complete |
| 集成令牌加密 Integration Tokens | AES-256-GCM 静态加密 At-rest AES-256-GCM encryption | ✅ 已实现 Implemented |
| 信号埋点 Signal Telemetry | append-only 事件日志,为日后校准采集显式/启发式信号 Append-only event log for future calibration | ✅ 已实现 Implemented |

## 项目状态 · Project Status

**核心后端机制已完成,正进入桌面/原生集成阶段。** SoulLedger 记忆系统、人格、思考队列、权限分级、向量检索、信号埋点均已实现并通过单元 + DB 集成 + 路由回归测试与一次端到端冒烟;剩余工作集中在桌面壳与 macOS 原生能力(EventKit)。
**Core backend mechanisms are complete; now moving into desktop/native integration.** The SoulLedger memory system, personality, thought queue, permission tiers, vector retrieval, and signal telemetry are implemented and covered by unit + DB-integration + route-regression tests plus an end-to-end smoke test. Remaining work centers on the desktop shell and macOS native capabilities (EventKit).

- ✅ Rust Axum 后端（API、WebSocket、数据库迁移 001–018）· Rust Axum backend (API, WebSocket, DB migrations 001–018)
- ✅ React + TypeScript + Vite 前端（聊天 + 记忆/人格/日历/思绪中心 + 反馈 UI）· React + TS + Vite frontend (chat + memory/personality/calendar/thought centers + feedback UI)
- ✅ PostgreSQL + Redis + Qdrant 开发环境 · development environment
- ✅ SoulLedger:记忆提取/去重、遗忘衰减 + 来源护栏、人格演化、离线 LLM 语义合并 · memory extraction/dedup, forgetting decay + provenance guard, personality evolution, offline LLM consolidation
- ✅ 语义记忆检索（Ollama 嵌入 + Qdrant，带 SQL 回退）· Semantic memory retrieval (Ollama embeddings + Qdrant, with SQL fallback)
- ✅ 思考队列状态机 + 防打扰 + 每日维护 · Thought queue state machine, anti-nag, daily maintenance
- ✅ 权限分级 L0–L4 + 审计日志 · Permission tiers L0–L4 + audit log
- ✅ 集成令牌 AES-256-GCM 静态加密 · Integration token at-rest encryption
- ✅ 信号埋点事件层 · Signal telemetry event layer
- ✅ Tauri 2 桌面壳（main + pet 窗口）· Tauri 2 desktop shell (main + pet windows)
- ◐ Apple Calendar:自然语言解析 + CRUD + L1 确认流(落 Postgres);**EventKit 真实写入待 Phase B** · NL parse + CRUD + L1 confirmation (persisted to Postgres); **real EventKit write pending Phase B**
- ⏳ WebSocket 实时通道仍为回声桩 · WebSocket realtime channel still an echo stub
...
| `LLM_API_KEY` | — | 云端 LLM API 密钥;设置后聊天/嵌入走 OpenAI 兼容端点,否则走 Ollama。· Cloud LLM API key; when set, chat/embeddings use the OpenAI-compatible endpoint, otherwise Ollama. |
| `LLM_API_BASE_URL` | — | OpenAI 兼容基础 URL;设置即启用云端后端。· OpenAI-compatible base URL; setting it switches to the cloud backend. |
| `LLM_EMBED_DIM` | `768` | 嵌入维度,需与嵌入模型匹配(Qdrant 集合大小)。· Embedding dimension; must match the embed model (Qdrant collection size). |
...
OpenAPI 3.0 规范由 [utoipa](https://github.com/juhaku/utoipa) 自动生成,已覆盖全部已实现路由(认证、聊天、记忆、人格、思绪、日历、权限、集成、信号等)。后端运行时可通过 `/swagger-ui` 访问 Swagger UI;严格的前后端契约验证(生成客户端 + 差异检查)仍在计划中。
OpenAPI 3.0 spec is generated by [utoipa](https://github.com/juhaku/utoipa) and now covers all implemented routes (auth, chat, memories, personality, thoughts, calendar, permissions, integrations, signals, …). Swagger UI is available at `/swagger-ui` when the backend is running; strict backend↔frontend contract validation (generated clients + diffing) is still planned.
