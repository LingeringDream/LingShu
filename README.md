...
**核心后端机制已完成，进入桌面/原生集成收尾阶段。** SoulLedger 记忆系统、人格、思考队列、权限分级、向量检索、信号埋点、工具调用、日历 EventKit 写入/删除等均已实现。后端含 **300+ 测试函数（16 项 DB 门控 ignored）**，前端新增 Vitest 套件（stores / lib / 组件，18 项）。
