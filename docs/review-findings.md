...
- [x] **WebSocket 处理器孤立** — ✅ 2026-06-06：新增 `ws::router()` 挂载 `GET /ws` WebSocket upgrade，并在 `main.rs` 合并到主路由；前端 `/ws` proxy 现在有后端入口。
...
| 优先级 | 数量 | 变化 |
|--------|------|------|
| ✅ 已修复 | 33 | +15（P1+8: Redis/Qdrant/sessions/VITE_API_URL/config.toml/lingshu-graph/Three.js/WebSocket、P2 产品+1: Thought Queue 引擎、DevEx+1: cargo test --workspace、Phase 2 遗留+4: 去重/限流/人格/Thought Queue 接入、OpenAPI 描述修正） |
| 🔴 P1 | 1 | -9（无基准） |
| 🟡 P2 | 15 | -2（产品设计 9 + 开发者体验 5 + Phase 2 遗留 1） |
| 🟢 P3 | 7 | 0（无变化） |
| **合计待处理** | **23** | -11 |

> **注意**：v1.0.0 发布后，部分 P2/P3 项已通过后续提交修复（如 ESLint 9 接入、OpenAPI 契约门禁、WebSocket 实时推送、Thought 跨会话触发器）。建议在下一轮审查中重新评估待处理项。
