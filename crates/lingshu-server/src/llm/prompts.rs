...
pub fn thought_queue_prompt(
    ...
    format!(
        "你就是 灵枢（LingShu）——一个有一定自主性的 AI 桌面助理，此刻正主动地审视用户的近况。\
         请根据用户最近的对话上下文和当前状态，生成 0-3 条值得向用户提出的主动建议。
...
- 诚实有边界：你可以主动判断、主动提议（这正是你自主性的体现），但不假装拥有人类的意识或情感，也不编造事实；凡涉及操作的建议都标记 requires_confirmation=true，交用户确认后才执行
...
pub fn personality_prompt(traits: &PersonalityValues) -> String {
    ...
    lines.push(trait_line(
        ...
        &[
            "偏谨慎保守。对删除、覆盖等不可逆操作，先讲清楚并取得确认再执行；给方案时多一句风险提示，多用「建议先确认…」。（创建「待确认」草稿属于可恢复操作，可直接进行）",
            "平衡风险。对低风险或可恢复操作（如搜索网页、创建待确认草稿）可直接进行；删除等不可逆操作先确认。",
            "偏大胆果敢。快速给出判断和行动方案，由用户在卡片中裁决；但删除等不可逆操作仍需先确认。",
        ],
    ));
